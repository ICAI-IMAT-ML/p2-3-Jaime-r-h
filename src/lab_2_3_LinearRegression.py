...
class LinearRegressor:
    ...
    def fit_multiple(self, X, y):
        ...
        Returns:
            None: Modifies the model's coefficients and intercept in-place.
        """
        X_b = np.c_[np.ones(X.shape[0]), X]  # Concatenamos una columna de 1s a X

        # Resolver la ecuación normal: β = (X^T X)^(-1) X^T y
        beta = np.linalg.inv(X_b.T @ X_b) @ X_b.T @ y

        # Extraer la intercepción y los coeficientes
    ...
    def predict(self, X):
        ...
        if np.ndim(X) == 1:
            ...
            for row in X:
                pred = self.intercept  # Empezamos con el intercepto
                for i in range(len(self.coefficients)):
                    pred += row[i] * self.coefficients[i]  # Producto escalar manual
...
def evaluate_regression(y_true, y_pred):
    ...
    """
    # R^2 Score
    ss_total = np.sum((y_true - np.mean(y_true)) ** 2)  # Suma de los cuadrados totales
    ss_residual = np.sum((y_true - y_pred) ** 2)  # Suma de los cuadrados residuales
